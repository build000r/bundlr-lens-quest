						...
						<ul className={`${isOpen ? '' : 'hidden'} z-10 mx-3 origin-top absolute right-0 lg:left-0 mt-1 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-200`} >
							{profiles &&
								profiles
									...
									.map((profile) => (
										<li className="cursor-pointer" onClick={() => {switchProfile(profile.id); setIsOpen((prev) => !prev)}}>

											<div key={profile.id} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-offset-2 focus:ring-offset-gray-100" tabIndex="-1">
